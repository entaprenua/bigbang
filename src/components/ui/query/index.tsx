...
type QuerySuccessProps = JSX.HTMLAttributes<HTMLDivElement>

const QuerySuccess = (props: QuerySuccessProps) => {
  const [local, others] = splitProps(props, ["class", "children"])
  const query = useQueryState()

  return (
    <Show when={query && query.isSuccess}>
      <div class={local.class} {...others}>
        {local.children}
      </div>
    </Show>
...
const MutationButton = (props: MutationButtonProps) => {
...
type MutationLoadingProps = JSX.HTMLAttributes<HTMLDivElement>

const MutationLoading = (props: MutationLoadingProps) => {
  const [local, others] = splitProps(props, ["class", "children"])
  const m = useMutationState()

  return (
    <Show when={m && m.isPending}>
      <div class={local.class} {...others}>
        {local.children ?? <div class="h-6 w-6 animate-spin rounded-full border-2 border-primary border-t-transparent" />}
      </div>
    </Show>
  )
}

type MutationErrorProps = JSX.HTMLAttributes<HTMLDivElement>

const MutationError = (props: MutationErrorProps) => {
  const [local, others] = splitProps(props, ["class", "children"])
  const m = useMutationState()
...
type MutationErrorMessageProps = JSX.HTMLAttributes<HTMLDivElement>

const MutationErrorMessage = (props: MutationErrorMessageProps) => {
  const [local, others] = splitProps(props, ["class"])
  const m = useMutationState()

  return (
    <div class={local.class} {...others}>
      {m?.error && (m.error as Error)?.message}
    </div>
  )
}
