...
export const wishlistsApi = {
  get: async (): Promise<Wishlist | null> => {
    const data = await executeGQL<{ wishlist: Wishlist | null }>(WISHLIST_QUERY)
    return data.wishlist ?? null
  },

  addItem: async (productId: string): Promise<boolean> => {
    const data = await executeGQL<{ addToWishlist: boolean }>(ADD_TO_WISHLIST_MUTATION, {
  ...
  removeItem: async (wishlistId: string): Promise<boolean> => {
    const data = await executeGQL<{ removeFromWishlist: boolean }>(REMOVE_FROM_WISHLIST_MUTATION, {
      wishlistId,
    })
