...
export const categoriesApi = {
  getAll: async (filters?: CategoryFilters): Promise<Category[]> => {
    ...
    const data = await executeGQL<{ categories: Category[] }>(CATEGORIES_QUERY, vars)
    return data.categories ?? []
  },

  getRoot: async (): Promise<Category[]> => {
    const data = await executeGQL<{ categories: Category[] }>(CATEGORIES_QUERY, {
      filters: { root: true },
    })
    return data.categories ?? []
  },
  ...
  getById: async (id: string): Promise<Category | null> => {
    const data = await executeGQL<{ category: Category | null }>(CATEGORY_QUERY, { id })
    return data.category ?? null
  },

  getBySlug: async (slug: string): Promise<Category | null> => {
    const data = await executeGQL<{ category: Category | null }>(CATEGORY_QUERY, { slug })
    return data.category ?? null
  },
}
